...
RSpec.describe RealtimeDepartureFetcher do
  ...
  describe "#stop_times" do
    let!(:applicable_stop_time) {
      create(:stop_time, agency: agency, stop: stop, trip: trip, departure_time: Duration.for_time(now + 10.minutes).to_s)
    }
    let!(:out_of_time_range_stop_time) {
      create(:stop_time, agency: agency, stop: stop, trip: trip, departure_time: Duration.for_time(now - 2.hours).to_s)
    }
    let!(:different_service_stop_time) {
      create(:stop_time, agency: agency, stop: stop, trip: create(:trip, agency: agency), departure_time: Duration.for_time(now).to_s)
    }
  ...
  describe "#departures" do
    let(:departure_time) { Duration.for_time(now + 10.minutes) }
    ...
    context "when a departure is less than 10 minutes past" do
      let(:departure_time) { Duration.for_time(now - 9.minutes) }
    ...
    context "when a departure is up to an hour in the past" do
      let(:departure_time) { Duration.for_time(now - 55.minutes) }
    ...
    context "when a departure is more than an hour in the past" do
      let(:departure_time) { Duration.for_time(now - 65.minutes ) }

      context "and realtime updates are available showing upcoming departure for the stop time" do
        let(:fake_stop_time_update) { OpenStruct.new(departure_time: Duration.for_time(now + 5.minutes)) }
